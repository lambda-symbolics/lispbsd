...
(defun test-bitmap ()
  ...
  (let ((bitmap (make-bitmap 8 4)))
    ...
    (test-assert (equal '("........" "........" "........" "........")
                        (bitmap-ascii bitmap)))
    (setf (bitmap-pixel bitmap 1 1) 255)
    (bitmap-fill bitmap :x 4 :y 0 :width 3 :height 2 :shade 255)
  ...
  (let ((box (make-bitmap 5 5)))
    ...
    (test-assert (equal '("#####" "#...#" "#...#" "#...#" "#####")
                        (bitmap-ascii box))))
  (let ((source (make-bitmap 3 3 :initial-element 255))
    ...
    (test-assert (= 255 (bitmap-pixel destination 0 0)))
    (test-assert (= 0 (bitmap-pixel destination 5 5))))
  (dolist (case '((:src 255 0 255)
                  (:ior 255 0 255)
                  (:ior 0 255 255)
                  (:ior 128 64 128)
                  (:xor 255 255 0)
  ...
  (let ((font *fixed-font*))
    ...
    (let ((expected (mapcar (lambda (row)
                              (map 'string
                                   ...
                                   row))
                            (rest (assoc #\! *fixed-font-art*))))
          (bitmap (make-bitmap 8 8 :initial-element 255)))
      (bitmap-draw-text bitmap font "!" :x 0 :y 0 :shade 0)
      (test-assert (equal expected (bitmap-ascii bitmap))))
    (let ((bitmap (make-bitmap 8 8)))
      (bitmap-draw-text bitmap font "ab" :x -4 :y -2)
      (test-assert (= 255 (bitmap-pixel bitmap 0 0)))
      (test-assert (= 255 (bitmap-pixel bitmap 5 0))))
    (let ((missing (make-missing-glyph 8 8)))
      (test-assert (= 255 (bitmap-pixel missing 0 0)))
      (test-assert (= 255 (bitmap-pixel missing 4 4)))
...
(defun test-window ()
  ...
  (let* ((*system-font* *fixed-font*)
    ...
    (test-assert (eq bottom (desktop-window-at desktop 10 8)))
    (bitmap-fill (window-content-bitmap bottom) :shade 255)
    (let ((screen (desktop-compose desktop)))
      (test-assert (= 0 (bitmap-pixel screen 0 0)) "background is white")
      (test-assert (= 255 (bitmap-pixel screen 23 7)) "shadow dither ink")
      (test-assert (= 0 (bitmap-pixel screen 22 7)) "shadow dither paper")
      (test-assert (= 255 (bitmap-pixel screen 6 18)) "shadow below the window")
      (test-assert (= 255 (bitmap-pixel screen 2 2)) "window border corner")
      (test-assert (= 255 (bitmap-pixel screen 21 17)) "window border far corner")
      (test-assert (= 255 (bitmap-pixel screen 19 4)) "focused title bar is ink")
      (test-assert (= 255 (bitmap-pixel screen 5 13)) "title separator line")
      (test-assert (= 255 (bitmap-pixel screen 3 14)) "content pixel is ink"))
    (window-show top)
    (let ((screen (desktop-compose desktop)))
      (test-assert (= 255 (bitmap-pixel screen 8 6)) "top window border corner")
    ...
    (unwind-protect
         (progn
           (let ((bitmap (make-bitmap 10 3)))
             ...
             (setf (bitmap-pixel bitmap 4 1) 255)
           ...
           (let* ((*system-font* *fixed-font*)
             ...
             (let ((screen (bitmap-read-pgm path)))
               (test-assert (= 24 (bitmap-width screen)))
               (test-assert (= 0 (bitmap-pixel screen 0 0))
                            "screenshot keeps the white background")
               (test-assert (= 255 (bitmap-pixel screen 22 8))
                            "screenshot keeps the drop shadow")
               (test-assert (= 255 (bitmap-pixel screen 2 2))
                            "screenshot keeps the window border")))
           (let ((gray (make-bitmap 3 2)))
             (setf (bitmap-pixel gray 1 0) 128)
             (setf (bitmap-pixel gray 2 1) 255)
             (bitmap-write-pgm gray path)
             (let ((read-back (bitmap-read-pgm path)))
               (test-assert (= 128 (bitmap-pixel read-back 1 0))
                            "pgm carries antialiased shades")
               (test-assert (= 255 (bitmap-pixel read-back 2 1)))
               (test-assert (= 0 (bitmap-pixel read-back 0 0)))))
...
(defun test-exec-window ()
  ...
  (let* ((*system-font* *fixed-font*)
    ...
    (labels ((type-string (string)
      ...
      (press-key ':up)
      (test-assert (string= "(oops" (exec-window-input exec-window))
                   "up recalls the previous input")
      (press-key ':up)
      (test-assert (string= "(+ 1 2)" (exec-window-input exec-window))
                   "up steps back through remembered inputs")
      (press-key ':up)
      (test-assert (string= "(+ 1 2)" (exec-window-input exec-window))
                   "recall clamps at the oldest input")
      (press-key ':down)
      (test-assert (string= "(oops" (exec-window-input exec-window)))
      (press-key ':down)
      (test-assert (string= "" (exec-window-input exec-window))
                   "stepping past the newest input clears the line"))))
...
(defun test-inspector-window ()
  ...
  (let* ((*system-font* *fixed-font*)
    ...
    (let ((content (window-content-bitmap window)))
      (test-assert (= 255 (bitmap-pixel content 100 11))
...
(defun test-window-close-box ()
  ...
  (let* ((*system-font* *fixed-font*)
    ...
    (test-assert (= 255 (bitmap-pixel (desktop-screen desktop) 32 6))
...
(defun test-system-font ()
  ...
  (let* ((desktop (make-desktop :width 400 :height 300))
    ...
    (desktop-compose desktop)
    (test-assert (= 255 (bitmap-pixel (desktop-screen desktop) 10 10))
  ...
  (let* ((*system-font* *fixed-font*)
    ...
    (let ((menu (desktop-open-menu desktop items :x 10 :y 10)))
      ...
      (let ((screen (desktop-screen desktop)))
        (test-assert (= 255 (bitmap-pixel screen 10 10)) "menu border")
        (test-assert (= 255 (bitmap-pixel screen 28 22))
...
(defun test-window-resize ()
  ...
  (let* ((*system-font* *fixed-font*)
    ...
    (let ((content (window-content-bitmap window)))
      ...
      (test-assert (= 255 (bitmap-pixel content 4 2))
...
(defun test-window-reshape ()
  ...
  (let* ((*system-font* *fixed-font*)
    ...
    (test-assert (= 255 (bitmap-pixel (window-content-bitmap window) 4 2))
